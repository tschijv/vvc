...
export const sections: PveSection[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { id: "80", naam: "Multi-organisatie toegang", prio: "eis", status: "yes", toelichting: "UserOrganisatie koppeltabel met BEHEERDER/RAADPLEGER rollen. Organisatie-switcher in de header. Admin kan meerdere organisaties per gebruiker toewijzen." },
  ...
  },
  {
    ...
    rows: [
      ...
      { id: "89", naam: "2FA / RBAC", prio: "eis", status: "yes", toelichting: "RBAC (10 rollen) + TOTP 2FA voor alle niet-admin rollen. Setup via /profiel/totp met QR-code. 6-digit invoer bij login. Admin exempt. API rate limiting (100/min API, 10/min auth, 30/min admin).", link: "/profiel/totp", linkTitle: "2FA instellen" },
  ...
  {
    title: "Security Audit (2026-03-26)",
    subtitle: "22 bevindingen — 16 opgelost, 2 deels, 3 open, 1 geaccepteerd",
    rows: [
      { id: "CVE-001", naam: "Auth ontbreekt op admin user routes", prio: "eis", status: "yes", toelichting: "GEFIXED: getSessionUser() + admin-rolcheck op alle /api/admin/users/ handlers (GET, POST, PUT, DELETE)." },
      { id: "CVE-002", naam: "Auth ontbreekt op CSV export routes", prio: "eis", status: "yes", toelichting: "GEFIXED: getSessionUser() toegevoegd aan alle 5 export routes (addenda, gemeenten, leveranciers, pakketten, pakketversies)." },
      { id: "CVE-003", naam: "Auth ontbreekt op addenda export", prio: "eis", status: "yes", toelichting: "GEFIXED: getSessionUser() check in /api/addenda/export/route.ts." },
      { id: "CVE-004", naam: "XSS via AI-adviseur (DOMPurify)", prio: "eis", status: "yes", toelichting: "GEFIXED: AIAdviseur.tsx sanitized HTML met DOMPurify.sanitize() voor dangerouslySetInnerHTML." },
      { id: "CVE-005", naam: "XSS via GlossaryHighlighter", prio: "eis", status: "yes", toelichting: "GEFIXED: GlossaryHighlighter.tsx gebruikt DOMPurify sanitize wrapper op alle dangerouslySetInnerHTML calls." },
      { id: "CVE-006", naam: "XSS via SVG KaartViewer", prio: "eis", status: "yes", toelichting: "GEFIXED: KaartViewer.tsx sanitized SVG met DOMPurify.sanitize(svg, { USE_PROFILES: { svg: true, svgFilters: true } })." },
      { id: "CVE-007", naam: "Ownership check addendum updates", prio: "eis", status: "yes", toelichting: "GEFIXED: PUT handler checkt leverancierId ownership, DELETE beperkt tot ADMIN." },
      { id: "CVE-008", naam: "Error messages lekken interne details", prio: "eis", status: "yes", toelichting: "GEFIXED: error.message vervangen door generieke 'Interne serverfout' in deploy, run-tests routes. Upload/kaart routes gebruiken custom error classes met veilige berichten." },
      { id: "CVE-009", naam: "2FA voor alle rollen inclusief admin", prio: "eis", status: "yes", toelichting: "GEFIXED: TOTP verificatie geldt voor alle gebruikers met totpEnabled, geen admin-uitzondering." },
      { id: "CVE-010", naam: "In-memory rate limiting op serverless", prio: "wens", status: "no", toelichting: "OPEN: Rate limiter Map leeft per instance. Redis/Vercel KV nodig voor productie-schaal." },
      { id: "CVE-011", naam: "Open redirect via callbackUrl", prio: "eis", status: "yes", toelichting: "GEFIXED: callbackUrl moet starten met '/' en niet met '//' — absolute URLs en protocol-relative URLs geweigerd." },
      { id: "CVE-012", naam: "Rate limiting op meer routes", prio: "wens", status: "partial", toelichting: "DEELS: 26 van 66 routes hebben withRateLimit. Auth en admin routes beschermd, sommige v1 endpoints nog niet." },
      { id: "CVE-013", naam: "File size limit op uploads", prio: "eis", status: "yes", toelichting: "GEFIXED: MAX_FILE_SIZE = 10MB in upload service met check voor verwerking." },
      { id: "CVE-014", naam: "MIME type validatie uploads", prio: "wens", status: "yes", toelichting: "GEFIXED: ALLOWED_MIMES whitelist (text/csv, application/json, xlsx) met validatie." },
      { id: "CVE-015", naam: "CSV injection in exports", prio: "wens", status: "yes", toelichting: "GEFIXED: escapeCsv() prefixed waarden die beginnen met =,+,-,@,\\t,\\r met enkele quote om Excel-formule-interpretatie te voorkomen." },
      { id: "CVE-016", naam: "Timing-safe Basic Auth", prio: "wens", status: "yes", toelichting: "GEFIXED: Constant-time string vergelijking via bitwise XOR in middleware (Edge Runtime compatible)." },
      { id: "CVE-017", naam: "Middleware sluit API uit van Basic Auth", prio: "wens", status: "no", toelichting: "OPEN: API routes niet beschermd door Basic Auth. Individuele auth checks per route compenseren dit." },
      { id: "CVE-018", naam: "CORS wildcard op RDF endpoints", prio: "wens", status: "nvt", toelichting: "GEACCEPTEERD: Access-Control-Allow-Origin: * op publieke Linked Data endpoints. Standaard voor open data." },
      { id: "CVE-019", naam: "Error logging bij catch blocks", prio: "wens", status: "yes", toelichting: "GEFIXED: Alle .catch(() => {}) vervangen door console.error() met beschrijvende melding in auth, favorieten, notificaties, begrippen sync." },
      { id: "CVE-020", naam: "Kwetsbare transitive dependencies", prio: "wens", status: "no", toelichting: "OPEN: hono, lodash, effect via Prisma. Niet direct runtime risico, afhankelijk van upstream fixes." },
      { id: "CVE-021", naam: "Audit logging uitbreiden", prio: "wens", status: "partial", toelichting: "DEELS: 26 logAudit() calls aanwezig. Profiel, favorieten, notificaties nog niet geaudit." },
      { id: "CVE-022", naam: "Content-Security-Policy header", prio: "eis", status: "yes", toelichting: "GEFIXED: CSP header met restrictieve policy in middleware. frame-ancestors 'none', unsafe-inline/eval voor Next.js compatibiliteit." },
    ],
  },
];
