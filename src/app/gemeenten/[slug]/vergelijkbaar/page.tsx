...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!gemeente) return {};
  return {
    title: `Vergelijkbare ${tenant.organisatieType.meervoud} met ${gemeente.naam}`,
    description: `Overzicht van alle ${tenant.organisatieType.meervoud} met overlap in applicatielandschap met ${gemeente.naam}`,
  };
}
    ...
    <main className="max-w-6xl mx-auto px-4 py-6">
      <Breadcrumbs
        items={[
          ...
          { label: `Vergelijkbare ${tenant.organisatieType.meervoud}`, href: `/gemeenten/${gemeente.id}/vergelijkbaar` },
        ]}
      />

      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">
          Vergelijkbare {tenant.organisatieType.meervoud} met {gemeente.naam}
        </h1>
        <p className="mt-2 text-sm text-gray-600 flex items-start gap-1.5">
          <span
            ...
            title={`Jaccard-similariteit: overlap = gedeelde pakketten / totaal unieke pakketten van beide ${tenant.organisatieType.meervoud} samen.`}
          >
            i
          </span>
          <span>
            Overlap is berekend met Jaccard-similariteit: het aantal gedeelde pakketten gedeeld door het totaal aantal unieke pakketten van beide {tenant.organisatieType.meervoud} samen. Hoe hoger het percentage, hoe meer overeenkomst in het applicatielandschap.
          </span>
        </p>
      </div>
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        <span className="text-sm text-gray-700">{tenant.organisatieType.meervoud} met minimaal 1 gedeeld pakket</span>
      </div>

      {similarOrganisaties.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-lg p-8 text-center text-gray-500">
          Geen vergelijkbare {tenant.organisatieType.meervoud} gevonden.
        </div>
      ) : (
